...
use crate::{utils::bool_to_emoji::bte, server_init::state_init::ServerState};
...
// Healthcheck GET 요청을 처리하는 함수.
// Processes Healthcheck GET request.
pub async fn healthcheck_handler(State(state): State<ServerState>) -> impl IntoResponse {
    ...
    let db_writer_url: &str = &state.db_url;
    let database_connection: bool = check_database_connection(&db_writer_url);
    ...
    checks_vec.push(database_connection);
    ...
    if all_good {
        message.push(format!("Database connection: {}", bte(database_connection)));
    ...
    if all_good {
        (StatusCode::OK, Json(healthcheck_reply))
    } else {
        (StatusCode::SERVICE_UNAVAILABLE, Json(healthcheck_reply))
    }
}

// DB URL 받아서 연결되는지 간단히 확인하는 함수.
// Get DB URL and check for connection.
pub fn check_database_connection(db_writer_url: &str) -> bool {
    match PgConnection::establish(db_writer_url) {
        Ok(conn) => {
            drop(conn);
            true
        }
