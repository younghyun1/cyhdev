...
export default function MediaCleanupPanel(props: Props) {
  ...
  const [pending, setPending] = createSignal<UnresolvedMediaCleanupItem | null>(
    null,
  );
  ...
  const loadRecords = async () => {
    ...
    setLoadError(null);
    try {
      const response = await service().unresolvedMediaCleanup(
        controller.signal,
      );
  ...
  return (
    <section
      id={ADMIN_OPERATION_SECTION_IDS.mediaCleanup}
      ...
      <div class="operations-panel-heading">
        <div>
          ...
          <Show when={records().length === 100}>
            <p class="operations-bound-note">
              {t("operations.media.bound_note")}
            </p>
          </Show>
        </div>
        <button
              ...
              <article class="operations-record">
                ...
                <dl class="operations-details">
                  ...
                  <div>
                    ...
                    <dd>
                      <code class="operations-inert-url">
                        {record.original_url}
                      </code>
                    </dd>
                  </div>
          ...
          <div class="operations-dialog-backdrop" role="presentation">
            ...
            >
              ...
              <form onSubmit={resolve}>
                <label for="cleanup-bucket">
                  {t("operations.media.bucket")}
                </label>
                <input
                  id="cleanup-bucket"
                ...
                <input
                  ...
                  required
                  onInput={(event) =>
                    setObjectKey(event.currentTarget.value)}
                />
                ...
                <div class="operations-dialog-actions">
                  ...
                  <button
                    ...
                    disabled={!confirmed() || objectKey().length === 0 ||
                      resolvingId() !== null}
                  >
                    {resolvingId() !== null
                      ? t("operations.media.reconciling")
                      : t("operations.media.reconcile")}
                  </button>
                </div>
              </form>
