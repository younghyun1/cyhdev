...
export default function HardPurgePanel(props: Props) {
  ...
  return (
    <section
      id={ADMIN_OPERATION_SECTION_IDS.hardPurge}
      class="operations-panel operations-danger-panel"
    ...
    >
      ...
      <form class="operations-purge-form" onSubmit={purge}>
        ...
        <Show when={userId().trim() !== "" && !isUuid(normalizedUserId())}>
          <p class="operations-field-error">
            {t("operations.purge.invalid_uuid")}
          </p>
        </Show>
        ...
        >
          {busy()
            ? t("operations.purge.purging")
            : t("operations.purge.submit")}
        </button>
      </form>
      <Show when={receipt()}>
        {(value) => (
          <section
            class="operations-purge-receipt"
            role="status"
            aria-labelledby="purge-receipt-heading"
          >
            <h3 id="purge-receipt-heading">
              {t("operations.purge.receipt_title")}
            </h3>
            <p
              class={value().profile_cleanup_remaining === 0
                ? "operations-receipt"
                : "operations-warning-receipt"}
            >
            ...
            <Show when={value().profile_cleanup_failures.length > 0}>
              ...
              <ul class="operations-failure-list">
                <For each={value().profile_cleanup_failures}>
                  {(failure) => (
                    <li>
...
function sanitizeReceipt(receipt: HardPurgeAccountResponse): PurgeReceipt {
  return {
    ...
    profile_cleanup_failures: receipt.profile_cleanup_failures.map((
      failure,
    ) => ({
