...
`build.sh` and `cargo xtask image` validate the pinned EU5 submodule before starting Docker: they refuse an uninitialized or conflicted submodule, a checkout at a commit other than the recorded gitlink, and uncommitted or untracked files inside it, because Docker copies that working tree. Both local optimized paths build the EU5 browser package inside the container, stage it into the frontend, retain only gzip-9 representations under `/eu5-locations-db/app/`, and compile those assets into the backend executable. If the submodule is absent or moved, restore it with `git submodule update --init --recursive` and rerun the command. `cargo xtask eu5-web-stage` still builds local submodule edits for development.
