...
SIGTERM or Ctrl-C stops both listeners from accepting and gives in-flight requests 10 seconds before axum-server closes the remaining connections. Upgraded WebSockets are not part of that drain and end when the process exits. After the listeners stop, `init/shutdown.rs` runs an ordered list of hooks with a shared 10-second budget and at most five seconds each; a hook that fails or overruns is logged and the next one still runs while budget remains. The current hooks flush the visitor-log buffer and the photograph view-count buffer. Add a hook to `server_shutdown_hooks` with a name and an async closure, in the order it must run, for example closing open calls before flushing counters those calls update.
