...
- Photograph cards reserve a cropped 4:3 mobile image area while detail sheets show the uncropped original. Mobile list requests use 12 records; desktop requests use 24. Detail maps, geosearch/upload code, and batch processing are lazy chunks. On desktop the detail map is prefetched below the route boundary for every visitor; upload and processing code is prefetched only once the viewer is known to be a superuser. Detail images support horizontal touch swipes.
