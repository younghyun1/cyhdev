...
fn blog_cache_case(database: &TestDatabase) -> DatabaseTestFuture<'_> {
    Box::pin(async move {
        ...
        context
            .accounts
            .assign_role(author.user_id, RoleType::Younghyun)
            .await?;
        ...
        require(
            search_hits
                .iter()
                .any(|post| post.post_id == created.post_id),
        ...
        require(
            old_hits.is_empty() && new_hits.iter().any(|post| post.post_id == created.post_id),
        ...
        require(
            cached.is_empty() && search_hits.is_empty(),
...
fn i18n_cache_case(database: &TestDatabase) -> DatabaseTestFuture<'_> {
    Box::pin(async move {
        ...
        .set((
            i18n_strings::i18n_string_content.eq(replacement),
        ...
        let stale = i18n
            ...
            .await?;
        require(
            stale.get("common.save") == before.as_ref(),
        ...
        require(
            refreshed
                .get("common.save")
                .is_some_and(|value| value == replacement),
