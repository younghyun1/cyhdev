...
use crate::{
    features::accounts::{
        ...
        service::{
            ...
            password_work::PasswordBudget, session_coordination::run_to_completion,
        },
    },
    util::{
...
impl AccountService {
    ...
    pub async fn soft_delete_account(
        ...
        run_to_completion(async move {
            // Deletion ends any authority the account holds, so it excludes authority leases.
            let authority_consistency = service.authority_consistency.write().await;
            ...
            let notification_schedule =
                RetentionNotificationSchedule::from_purge_after(purge_after)
                    .ok_or(AccountError::RetentionScheduleOverflow)?;
            let receipt = service
                .repository
                .soft_delete_account(
    ...
    pub async fn hard_purge_account(
        ...
        let service = Arc::clone(self);
        run_to_completion(async move {
            let retention_delivery = service.retention_notification_delivery_gate.write().await;
            let authority_consistency = service.authority_consistency.write().await;
            let session_consistency = service.session_consistency.write().await;
            let receipt = service
                .repository
