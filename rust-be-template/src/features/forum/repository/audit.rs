...
use crate::{
    features::{
        ...
        forum::{
            domain::{
                models::{
                    ForumModerationAuditPage, ForumModerationAuditView, ForumTimestampCursor,
                },
                validation::ForumPageSize,
            },
            error::ForumError,
            repository::{
                forum_repository::ForumRepository,
                records::{ForumAuditRow, audit_from_row},
            },
        },
    },
...
impl ForumRepository {
    pub async fn moderation_audit_page(
        ...
        let mut query = forum_moderation_audit_events::table
            ...
            .order((
                forum_moderation_audit_events::forum_moderation_audit_event_created_at.desc(),
                forum_moderation_audit_events::forum_moderation_audit_event_id.desc(),
            ))
            .into_boxed();
        ...
        let mut rows = query
            ...
            .load::<ForumAuditRow>(&mut connection)
            .await?;
