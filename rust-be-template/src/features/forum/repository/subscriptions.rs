...
use crate::{
    features::forum::{
        error::ForumError,
        repository::{enums::DbForumContentState, forum_repository::ForumRepository, records::NewForumSubscriptionRecord},
    },
...
impl ForumRepository {
    pub async fn subscribe(
        ...
        connection.transaction::<bool, ForumError, _>(async move |connection| {
...
pub(super) async fn ensure_subscription(
    ...
    if count >= MAX_FORUM_TOPIC_SUBSCRIPTIONS {
        return Err(ForumError::SubscriptionSaturated { maximum: MAX_FORUM_TOPIC_SUBSCRIPTIONS });
    }
...
async fn lock_topic(connection: &mut AsyncPgConnection, topic_id: Uuid) -> Result<(), ForumError> {
    let topic = forum_topics::table
        ...
        .for_update().first::<DbForumContentState>(&mut *connection).await.optional()?;
    match topic {
