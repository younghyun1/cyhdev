...
use crate::{
    features::forum::{
        domain::{
            enums::ForumModerationAction, models::ForumModerationReceipt,
            validation::ForumModerationReason,
        },
        error::ForumError,
        repository::{
            enums::{DbForumContentState, DbForumModerationAction, DbForumTopicAccessState},
            forum_repository::ForumRepository,
            records::NewForumModerationAuditRecord,
            replies::lock_reply,
            topics::lock_topic,
        },
    },
...
impl ForumRepository {
    pub async fn moderate_topic(
        ...
        connection
            .transaction::<ForumModerationReceipt, ForumError, _>(async move |connection| {
                ...
                match action {
                    ForumModerationAction::TopicHidden
                        if content_state == DbForumContentState::Visible =>
                    {
                        content_state = DbForumContentState::Hidden;
                        hidden_at = Some(now);
                    }
                    ForumModerationAction::TopicRestored
                        if content_state == DbForumContentState::Hidden =>
                    {
                        content_state = DbForumContentState::Visible;
                        hidden_at = None;
                    }
                    ForumModerationAction::TopicLocked
                        if access_state == DbForumTopicAccessState::Open =>
                    {
                        access_state = DbForumTopicAccessState::Locked
                    }
                    ForumModerationAction::TopicUnlocked
                        if access_state == DbForumTopicAccessState::Locked =>
                    {
                        access_state = DbForumTopicAccessState::Open
                    }
                    ...
                    ForumModerationAction::ReplyHidden | ForumModerationAction::ReplyRestored => {
                        return Err(ForumError::ContentStateConflict);
                    }
                ...
                .set((
                    forum_topics::forum_topic_content_state.eq(content_state),
                    forum_topics::forum_topic_access_state.eq(access_state),
                    forum_topics::forum_topic_is_pinned.eq(is_pinned),
                    forum_topics::forum_topic_hidden_at.eq(hidden_at),
                    forum_topics::forum_topic_revision.eq(forum_topics::forum_topic_revision + 1),
                    forum_topics::forum_topic_updated_at.eq(now),
                ))
                .returning(forum_topics::forum_topic_revision)
                .get_result::<i32>(&mut *connection)
                .await?;
                let audit_event_id = insert_audit(
                    connection,
                    NewForumModerationAuditRecord {
    ...
    pub async fn moderate_reply(
        ...
        connection
            .transaction::<ForumModerationReceipt, ForumError, _>(async move |connection| {
                ...
                if record.forum_reply_revision != expected_revision {
                    return Err(ForumError::RevisionConflict);
                }
                if record.forum_reply_content_state == DbForumContentState::Deleted {
                    return Err(ForumError::ContentStateConflict);
                }
                let (content_state, hidden_at) = match action {
                    ForumModerationAction::ReplyHidden
                        if record.forum_reply_content_state == DbForumContentState::Visible =>
                    {
                        (DbForumContentState::Hidden, Some(now))
                    }
                    ForumModerationAction::ReplyRestored
                        if record.forum_reply_content_state == DbForumContentState::Hidden =>
                    {
                        (DbForumContentState::Visible, None)
                    }
                    ForumModerationAction::TopicHidden
                    | ForumModerationAction::TopicRestored
                    | ForumModerationAction::TopicLocked
                    | ForumModerationAction::TopicUnlocked
                    | ForumModerationAction::TopicPinned
                    | ForumModerationAction::TopicUnpinned => {
                        return Err(ForumError::ContentStateConflict);
                    }
                    _ => return Err(ForumError::NoChange),
                };
                let revision = diesel::update(
                    forum_replies::table.filter(forum_replies::forum_reply_id.eq(reply_id)),
                )
                .set((
                    forum_replies::forum_reply_content_state.eq(content_state),
                    forum_replies::forum_reply_hidden_at.eq(hidden_at),
                    forum_replies::forum_reply_revision.eq(forum_replies::forum_reply_revision + 1),
                    forum_replies::forum_reply_updated_at.eq(now),
                ))
                .returning(forum_replies::forum_reply_revision)
                .get_result::<i32>(&mut *connection)
                .await?;
                let audit_event_id = insert_audit(
                    connection,
