...
use crate::{
    ...
    schema::{
        forum_notifications, forum_replies, forum_topic_subscriptions, forum_topics,
        sql_types::ForumNotificationKind as ForumNotificationKindSql,
    },
};
    ...
    pub async fn create_reply(
        ...
        connection
            .transaction::<ForumMutationReceipt, ForumError, _>(async move |connection| {
                ...
                if topic_state.0 != DbForumContentState::Visible {
                    return Err(ForumError::ContentStateConflict);
                }
                if topic_state.1 != DbForumTopicAccessState::Open {
                ...
                let (reply_id, revision, updated_at) = diesel::insert_into(forum_replies::table)
                    .values(NewForumReplyRecord {
                        forum_reply_topic_id: topic_id,
                        forum_reply_author_user_id: actor_user_id,
                        forum_reply_body: body.as_ref(),
                        forum_reply_created_at: now,
                        forum_reply_updated_at: now,
                    })
                    .returning((
                        forum_replies::forum_reply_id,
                        forum_replies::forum_reply_revision,
                        forum_replies::forum_reply_updated_at,
                    ))
                ...
                .set((
                    forum_topics::forum_topic_reply_count
                        .eq(forum_topics::forum_topic_reply_count + 1),
    ...
    pub async fn delete_reply(
        ...
        connection
            .transaction::<ForumMutationReceipt, ForumError, _>(async move |connection| {
                ...
                .set((
                    forum_replies::forum_reply_body.eq(DELETED_CONTENT_BODY),
                    forum_replies::forum_reply_content_state.eq(DbForumContentState::Deleted),
                    forum_replies::forum_reply_hidden_at.eq(Option::<DateTime<Utc>>::None),
                    forum_replies::forum_reply_deleted_at.eq(Some(now)),
                    forum_replies::forum_reply_revision.eq(forum_replies::forum_reply_revision + 1),
                    forum_replies::forum_reply_updated_at.eq(now),
                ))
                .returning((
...
pub(super) async fn lock_reply(
    connection: &mut diesel_async::AsyncPgConnection,
    reply_id: Uuid,
) -> Result<ForumReplyRecord, ForumError> {
    forum_replies::table
        .filter(forum_replies::forum_reply_id.eq(reply_id))
        .select(ForumReplyRecord::as_select())
        .for_update()
        .first::<ForumReplyRecord>(&mut *connection)
        .await
        .optional()?
        .ok_or(ForumError::ReplyNotFound)
}

fn validate_author_reply(
    record: &ForumReplyRecord,
    actor_user_id: Uuid,
    expected_revision: i32,
) -> Result<(), ForumError> {
    if record.forum_reply_author_user_id != actor_user_id {
