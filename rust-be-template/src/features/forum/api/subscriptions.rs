...
use crate::{
    dto::responses::{forum::topics::ForumSubscriptionResponse, response_data::http_resp},
...
pub async fn subscribe_forum_topic(
    ...
    let subscribed = state
        .forum_service()
        .set_subscription(user_id, topic_id, true)
        .await
...
pub async fn unsubscribe_forum_topic(
    Extension(user_id): Extension<Uuid>,
    State(state): State<Arc<ServerState>>,
    Path(topic_id): Path<Uuid>,
) -> HandlerResponse<impl IntoResponse> {
    ...
    let subscribed = state
        .forum_service()
        .set_subscription(user_id, topic_id, false)
        .await
        .map_err(map_forum_error)?;
    Ok(http_resp(
        ForumSubscriptionResponse {
