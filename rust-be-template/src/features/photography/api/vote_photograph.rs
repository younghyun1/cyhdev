...
pub async fn vote_photograph(Extension(user_id): Extension<Uuid>, State(state): State<Arc<ServerState>>, Path(photograph_id): Path<Uuid>, Json(request): Json<VotePhotographRequest>) -> HandlerResponse<impl IntoResponse> {
    let start = tokio_now();
    let counts = state.photography_service().vote_photograph(user_id, photograph_id, request.is_upvote).await.map_err(map_insertion_error)?;
