...
responses((status = 200, body = PhotographCommentResponse), (status = 401, body = CodeErrorResp), (status = 413, description = "Request body too large", body = CodeErrorResp), (status = 404, body = CodeErrorResp)))]
pub async fn update_photograph_comment(
    ...
    BoundedJson(request): SocialJson<UpdatePhotographCommentRequest>,
