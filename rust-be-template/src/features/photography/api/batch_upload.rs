...
use crate::{
    ...
    features::photography::{
        domain::photograph::PhotographContext,
        service::batch_upload::{
            BatchMetadata, MAX_FILE_SIZE_BYTES, MAX_FILES_PER_BATCH, StagedBatchFile,
        },
    },
        ...
        .map_err(|error| code_err(CodeError::FILE_UPLOAD_ERROR, error))?
    {
        match field.name().map(str::to_owned).as_deref() {
            Some("files") | Some("file") => {
                ...
                if !content_type.as_deref().is_some_and(is_allowed_image_mime) {
                    return Err(code_err(
                ...
                loop {
                    let chunk = match field.chunk().await {
                        Ok(Some(chunk)) => chunk,
                        Ok(None) => break,
                        Err(error) => return Err(code_err(CodeError::FILE_UPLOAD_ERROR, error)),
                    };
                    let chunk_size = u64::try_from(chunk.len()).map_err(|_| {
                        code_err(CodeError::FILE_UPLOAD_ERROR, "Batch chunk size overflow")
                    })?;
            ...
            Some("meta") => {
                let text = read_bounded_text_field(field, MAX_BATCH_META_BYTES)
                    .await
                    .map_err(|error| code_err(CodeError::FILE_UPLOAD_ERROR, error))?;
                let parsed = serde_json::from_str::<Vec<BatchMetaEntry>>(&text)
                    .map_err(|error| code_err(CodeError::INVALID_REQUEST, error))?;
                metadata = Some(
                    parsed
                        .into_iter()
                        .map(|value| BatchMetadata {
            ...
            Some("context") | Some("photograph_context") => {
                let text = read_bounded_text_field(field, MAX_CONTEXT_BYTES)
                    .await
                    .map_err(|error| code_err(CodeError::FILE_UPLOAD_ERROR, error))?;
                context = PhotographContext::parse(&text).ok_or_else(|| {
                    code_err(CodeError::INVALID_REQUEST, "Invalid photograph context")
                })?;
            }
