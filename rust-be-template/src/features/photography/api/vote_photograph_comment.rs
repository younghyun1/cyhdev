...
responses((status = 200, body = VotePhotographResponse), (status = 401, body = CodeErrorResp), (status = 413, description = "Request body too large", body = CodeErrorResp), (status = 500, body = CodeErrorResp)))]
pub async fn vote_photograph_comment(
    ...
    let counts = state
        .photography_service()
        .vote_comment(user_id, comment_id, request.is_upvote)
        .await
