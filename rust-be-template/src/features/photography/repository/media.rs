...
use crate::{
    features::photography::{
        domain::photograph::{NewPhotograph, Photograph},
        error::PhotographyError,
        repository::{
            photography_repository::PhotographyRepository,
            records::{NewPhotographRecord, PhotographRecord},
        },
    },
    persistence::{
    ...
    schema::photographs,
    util::media::cleanup::{
        EnqueuedMediaCleanup, MediaCleanupRequest, REASON_DELETED_PHOTOGRAPH_IMAGE,
        REASON_DELETED_PHOTOGRAPH_THUMBNAIL,
    },
};

pub(crate) struct RetiredPhotographs {
    pub(crate) cleanup: EnqueuedMediaCleanup,
    pub(crate) deleted_rows: usize,
}

impl PhotographyRepository {
    pub async fn insert_photograph(
        &self,
        user_id: Uuid,
        ...
        connection
            .transaction::<Photograph, PhotographyError, _>(async move |connection| {
                lock_superuser(connection, user_id).await?;
                let row = diesel::insert_into(photographs::table)
                    .values(NewPhotographRecord::from(photograph))
                    .returning(PhotographRecord::as_returning())
                    .get_result::<PhotographRecord>(&mut *connection)
                    .await?;
                Ok(row.into())
            })
            .await
    ...
    pub(crate) async fn retire_photographs(
        &self,
        requester_id: Uuid,
        photograph_ids: &[Uuid],
    ) -> Result<RetiredPhotographs, PhotographyError> {
        let mut connection = self.connection().await?;
        connection
            .transaction::<RetiredPhotographs, PhotographyError, _>(async move |connection| {
    ...
    pub async fn user_is_deleted(&self, user_id: Uuid) -> Result<bool, PhotographyError> {
        let mut connection = self.connection().await?;
        Ok(load_deleted_user_ids(&mut connection, &[user_id])
            .await?
            .contains(&user_id))
    }
}
...
async fn lock_superuser(
    ...
    lock_active_superuser(connection, user_id)
        .await
