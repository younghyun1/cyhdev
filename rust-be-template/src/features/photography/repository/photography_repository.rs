...
#[derive(Clone)]
pub struct PhotographyRepository { pool: Pool<AsyncPgConnection> }

impl PhotographyRepository {
    pub fn new(pool: Pool<AsyncPgConnection>) -> Self { Self { pool } }
    pub(super) async fn connection(&self) -> Result<PooledConnection<'_, AsyncPgConnection>, PhotographyError> {
