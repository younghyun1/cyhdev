...
use super::search_index::PostSearchIndex;
use crate::features::blog::service::queries::{
...
impl PostSearchIndex {
    ...
    fn tokenize_title_query(&self, query_str: &str) -> anyhow::Result<Vec<String>> {
        ...
        let field_entry = schema.get_field_entry(self.inner.title_field);
    ...
    fn build_title_query(&self, query_str: &str) -> anyhow::Result<Box<dyn tantivy::query::Query>> {
        ...
        if query_str.split_whitespace().count() == 1 {
            ...
            if tokens.len() == 1 {
                let term = tantivy::Term::from_field_text(self.inner.title_field, &tokens[0]);
    ...
    fn collect_post_ids(
        ...
        let offset = offset.min(BLOG_SEARCH_MAX_OFFSET);
        let limit = limit.min(BLOG_SEARCH_MAX_LIMIT);
        ...
        for (_score, doc_address) in top_docs {
            ...
            if let Some(post_id_value) = retrieved_doc.get_first(self.inner.post_id_field)
