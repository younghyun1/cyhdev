...
use super::{
    super::{
        domain::{
            actor::ChatActor,
            ban::LiveChatBan,
            message::{DEFAULT_LIVE_CHAT_ROOM, LiveChatMessage},
        },
        error::LiveChatError,
    },
...
impl LiveChatRepository {
    ...
    pub async fn user_presentation(
        ...
        if ids.is_empty() {
            return Ok(UserPresentation {
                ...
                profile_urls: HashMap::new(),
                deleted_user_ids: HashSet::new(),
            });
        }
        ...
        let rows = users::table
            ...
            .load::<(Uuid, i32, Option<chrono::DateTime<Utc>>, Option<String>)>(&mut connection)
            .await?;
        let mut country_codes = HashMap::new();
        let mut profile_urls = HashMap::new();
        for (id, country, deleted, profile) in rows {
            if deleted.is_none() {
                deleted_user_ids.remove(&id);
                country_codes.insert(id, country);
                if let Some(profile) = profile {
                    profile_urls.insert(id, profile);
                }
            }
        }
        Ok(UserPresentation {
    ...
    pub async fn insert_message(
        ...
        actor: &ChatActor,
        body: String,
    ) -> Result<LiveChatMessage, LiveChatError> {
        let mut connection = self.connection().await?;
        connection
            .transaction::<LiveChatMessage, LiveChatError, _>(async move |connection| {
                if let Some(user_id) = actor.user_id {
                    lock_active_user(connection, user_id).await?;
                }
                diesel::insert_into(live_chat_messages::table)
                    .values(NewMessageRecord {
                    ...
                    .get_result::<MessageRecord>(&mut *connection)
                    .await
                    .map(LiveChatMessage::from)
            ...
            .transaction::<LiveChatBan, LiveChatError, _>(async move |connection| {
                ...
                diesel::insert_into(live_chat_bans::table)
                    .values(NewBanRecord {
                        ...
                        user_id: actor.user_id,
                        banned_ip: Some(ipnet::IpNet::from(ip)),
                        reason: "More than 10 live chat message events in one second.".to_owned(),
                        ban_source:
                            super::super::domain::ban::LIVE_CHAT_BAN_SOURCE_ABNORMAL_MESSAGING
                                .to_owned(),
                        banned_at: Utc::now(),
                        expires_at: None,
                    })
    ...
    pub async fn active_ban_for(
        ...
        let mut connection = self.connection().await?;
        let now = Utc::now();
        let network = ipnet::IpNet::from(ip);
        ...
        query = match user_id {
            Some(id) => query.filter(
                live_chat_bans::user_id
                    .eq(Some(id))
                    .or(live_chat_bans::banned_ip.eq(Some(network))),
            ),
