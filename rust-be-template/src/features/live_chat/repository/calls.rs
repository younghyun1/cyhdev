...
impl LiveChatRepository {
    pub async fn open_call(
        ...
        connection
            .transaction::<Uuid, LiveChatError, _>(async move |connection| {
                if let Some(user_id) = user_id {
                    lock_active_user_for_call(connection, user_id).await?;
                }
                diesel::insert_into(live_chat_calls::table)
                    .values(NewCallRecord {
            ...
            .transaction::<Uuid, LiveChatError, _>(async move |connection| {
                ...
                diesel::insert_into(live_chat_call_participants::table)
                    .values(NewParticipantRecord {
                        ...
                        participant_had_audio: audio,
                        participant_had_video: video,
                    })
                    .execute(&mut *connection)
                    .await?;
                Ok(participant_id)
