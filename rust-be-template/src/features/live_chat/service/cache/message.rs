...
pub struct CachedChatMessage {
    ...
    /// Server-side only (geo flag lookup and anonymization); never serialized.
    #[serde(skip)]
...
impl CachedChatMessage {
    /// Convert a persisted row for presentation.
    ///
    /// Guest names always derive from the keyed identity rather than the stored
    /// text: rows written before keyed nicknames hold an unkeyed IP-derived name
    /// (or a literal `guest@<ip>` label), and a guest's history should read as
    /// one person for as long as the identity secret is stable.
    pub fn from_persisted(message: LiveChatMessage, identity_key: &GuestIdentityKey) -> Self {
        let guest_ip = message.guest_ip.map(|ip| ip.addr().to_canonical());
        let sender_display_name = match guest_ip {
            Some(ip) if message.sender_kind == LIVE_CHAT_SENDER_KIND_GUEST => {
                identity_key.identify(ip).nickname
            }
            _ => message.sender_display_name,
        };
